...
inactivation = pd.read_csv("input/reaction_inactivation.csv", sep=";")
mutation = ''


for num, id in enumerate(inactivation.reaction):
    if inactivation.status[num] == 'off':
        lb_mut[mnet.reactions_id.index(id+'_l')] = 0
        lb_mut[mnet.reactions_id.index(id+'_s')] = 0
        lb_mut[mnet.reactions_id.index(id+'_r')] = 0
        ub_mut[mnet.reactions_id.index(id+'_l')] = 0
        ub_mut[mnet.reactions_id.index(id+'_s')] = 0
        ub_mut[mnet.reactions_id.index(id+'_r')] = 0
        mutation = mutation + '_' + id + '_' + 'off'
        
    if inactivation.status[num] == 'red':
        if x2[mnet.reactions_id.index(id+'_l')] >= 0:
            ub_mut[mnet.reactions_id.index(id+'_l')] = x2[mnet.reactions_id.index(id+'_l')]*0.5
            lb_mut[mnet.reactions_id.index(id+'_l')] = 0.0
        else:
            lb_mut[mnet.reactions_id.index(id+'_l')] = x2[mnet.reactions_id.index(id+'_l')]*0.5
            ub_mut[mnet.reactions_id.index(id+'_l')] = 0.0

        if x2[mnet.reactions_id.index(id+'_s')] >= 0:
            ub_mut[mnet.reactions_id.index(id+'_s')] = x2[mnet.reactions_id.index(id+'_l')]*0.5
            lb_mut[mnet.reactions_id.index(id+'_s')] = 0.0
        else:
            lb_mut[mnet.reactions_id.index(id+'_s')] = x2[mnet.reactions_id.index(id+'_l')]*0.5
            ub_mut[mnet.reactions_id.index(id+'_s')] = 0.0

        if x2[mnet.reactions_id.index(id+'_r')] >= 0:
            ub_mut[mnet.reactions_id.index(id+'_r')] = x2[mnet.reactions_id.index(id+'_l')]*0.5
            lb_mut[mnet.reactions_id.index(id+'_r')] = 0.0
        else:
            lb_mut[mnet.reactions_id.index(id+'_r')] = x2[mnet.reactions_id.index(id+'_l')]*0.5
            ub_mut[mnet.reactions_id.index(id+'_r')] = 0.0

        mutation = mutation + '_' + id + '_' + 'red'
...
try:
    ...
    probFBA3 = cplex.Cplex()

    probFBA3.set_problem_name("FBATomatoMinPhotons")

    probFBA3.objective.set_sense(probFBA.objective.sense.minimize)

    probFBA3.linear_constraints.add(rhs=rhs + addrhs, senses=sense + addsense,
                                   names=["QSSA_" + id for id in mnet.metabolites_id] + addnames)

    probFBA3.variables.add(obj=obj, lb=lb_mut, ub=ub_mut, names=mnet.reactions_id + mnet.exchangereactions_id)

    probFBA3.linear_constraints.set_coefficients(
        zip(mnet.stoichMat_rows + mnet.exchangestoichMat_rows + addstoichMat_rows,
            mnet.stoichMat_columns + mnet.exchangestoichMat_columns + addstoichMat_columns,
            mnet.stoichMat_values + mnet.exchangestoichMat_values + addstoichMat_values))

    probFBA3.write("output/LP/" + "FBATomatoMinPhotons" + "_probFBA.lp")

    probFBA3.solve()

    x3 = probFBA3.solution.get_values()


    # Print results
    print("Solution status = ", probFBA3.solution.get_status(), ":",)
    status_3 = probFBA3.solution.get_status()
    print(probFBA3.solution.status[probFBA3.solution.get_status()])
    print("Objective value  = ", probFBA3.solution.get_objective_value())
    print("Iteration count = ", probFBA3.solution.progress.get_num_iterations())


    # Create new constraints for FBA 2)

    # define new bounds for FBA 2)
    lb2_mut = lb_mut + [-cplex.infinity] * len(mnet.reactions_id)

    ub2_mut = ub_mut + [cplex.infinity] * len(mnet.reactions_id)

    #add constraints for photon uptake from previous FBA
    lb2_mut[mnet.reactions_id.index('R_EX_photon_h_s')] = x3[mnet.reactions_id.index('R_EX_photon_h_s')]
    ub2_mut[mnet.reactions_id.index('R_EX_photon_h_s')] = x3[mnet.reactions_id.index('R_EX_photon_h_s')]

    lb2_mut[mnet.reactions_id.index('R_EX_photon_h_l')] = x3[mnet.reactions_id.index('R_EX_photon_h_l')]
    ub2_mut[mnet.reactions_id.index('R_EX_photon_h_l')] = x3[mnet.reactions_id.index('R_EX_photon_h_l')]

    # define new rhs for FBA 2)
    rhs3 = rhs + addrhs + [0] * len(mnet.reactions_id) * 2

    # define new sense for FBA 2)
    sense3 = sense + addsense + "L" * len(mnet.reactions_id) * 2

    # define new obj FBA 2)
    obj3 = [0]*(len(mnet.reactions_id) + len(mnet.exchangereactions_id)) + [1] * len(mnet.reactions_id)
    ...
    try:
        ...
        probFBA4 = cplex.Cplex()

        probFBA4.set_problem_name("FBATomatoMinFluxSum")

        probFBA4.objective.set_sense(probFBA4.objective.sense.minimize)

        probFBA4.linear_constraints.add(rhs=rhs3, senses=sense3,
                                        names=["QSSA_" + id for id in mnet.metabolites_id] + addnames + minnames)

        probFBA4.variables.add(obj=obj3, lb=lb2_mut, ub=ub2_mut,
                               names=mnet.reactions_id + mnet.exchangereactions_id
                                     + ["MinVar_" + id for id in mnet.reactions_id])
        probFBA4.linear_constraints.set_coefficients(
            zip(mnet.stoichMat_rows + mnet.exchangestoichMat_rows + addstoichMat_rows + minstoichMat_rows,
                mnet.stoichMat_columns + mnet.exchangestoichMat_columns + addstoichMat_columns + minstoichMat_columns,
                mnet.stoichMat_values + mnet.exchangestoichMat_values + addstoichMat_values + minstoichMat_values))

        probFBA4.write("output/LP/" + "FBATomatoMinFluxSum" + "_probFBA.lp")

        probFBA4.solve()

        x4 = probFBA4.solution.get_values()

        # Print results
        print("Solution status = ", probFBA4.solution.get_status(), ":", )
        print(probFBA4.solution.status[probFBA4.solution.get_status()])
        print("Objective value  = ", probFBA4.solution.get_objective_value())
        print("Iteration count = ", probFBA4.solution.progress.get_num_iterations())
        ...
        if status_3 == 1:
            df1 = pd.DataFrame({
                ...
                'FBA value WT': x2[0:len(mnet.reactions_name + mnet.exchangereactions_name)],
                'Absolute FBA value WT:': [abs(v) for v in x2[0:len(mnet.reactions_name + mnet.exchangereactions_name)]],
                'FBA value MUT': x4[0:len(mnet.reactions_name + mnet.exchangereactions_name)],
                'Absolute FBA value MUT:': [abs(v) for v in x4[0:len(mnet.reactions_name + mnet.exchangereactions_name)]]})
        else:
            df1 = pd.DataFrame({
                'Reaction Name': mnet.reactions_name + mnet.exchangereactions_name,
                'Reaction Formula Name': mnet.reactions_formulasNames + mnet.exchangereactions_formulasNames,
                ...
                'FBA value WT': x2[0:len(mnet.reactions_name + mnet.exchangereactions_name)],
                'Absolute FBA value WT:': [abs(v) for v in x2[0:len(mnet.reactions_name + mnet.exchangereactions_name)]],
                'FBA value MUT': ['growth_infeasible' for k in (mnet.reactions_name + mnet.exchangereactions_name)],
                'Absolute FBA value MUT:': ['growth_infeasible' for k in (mnet.reactions_name + mnet.exchangereactions_name)]})

        val_atp = str(mnet.cost)
        df1.to_excel('output/FBA_Reaction_Inactivation_' +
                     'TransportCost_' + val_atp +
                     '_RatioPS_' + str(mnet.leaf_stem_photosynthesis) +
                     '_Xyl_' + str(mnet.xyl) + mutation + '.xlsx',
