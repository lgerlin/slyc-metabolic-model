...
#no3 uptake only
lb[mnet.nb_reac + mnet.exchangereactions_id.index('Exch_'+'M_nh4_b_r')] = 0

#############################################################################

#                      FLUX BALANCE ANALYSIS
#                     1) Photon uptake minimization
#                     2) Biomass maximization with nitrogen uptake constrained
...
try:
    ...
    no3_uptake_root = x[mnet.reactions_id.index('R_EX_no3_c_r')]
...
sense2 = sense + addsense

# define new obj FBA 2)
obj2 = [0]*(len(mnet.reactions_id) + len(mnet.exchangereactions_id))
obj2[mnet.reactions_id.index('R_BIOMASS_LEAF_l')] = 1

#generate several nitrogen limitations situations
red_nitrogen = [i for i in np.arange(1.0, 8.02, 0.02)]
gr_list = []

#remove the fixed leaf biomass growth rate
lb2[mnet.reactions_id.index('R_BIOMASS_LEAF_l')] = 0.0

for red in red_nitrogen:

    print(red)
    #constraint on nitrate uptake
    ub2[mnet.reactions_id.index('R_EX_no3_c_r')] = no3_uptake_root / red

    try:
        #####################################
        # FBA 2) Biomass maximization with nitrogen uptake constrained
        #####################################

        probFBA2 = cplex.Cplex()

        probFBA2.set_problem_name("FBATomatoMaxBiom")

        probFBA2.objective.set_sense(probFBA2.objective.sense.maximize)
        ...
        probFBA2.write("output/LP/" + "FBATomatoMaxBiom" + "_probFBA.lp")
        ...
        gr_plant = 0.24 * (x2[mnet.reactions_id.index('R_BIOMASS_LEAF_l')] / 0.21)
...
# save results
df1 = pd.DataFrame({'nitrogen red': red_nitrogen,
                    'plant GR': gr_list})

df1.to_excel('output/FBA_nitrogen_reduction.xlsx',
             sheet_name='results_nitrogen_reduction', index=False)