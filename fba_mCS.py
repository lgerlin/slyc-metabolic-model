...
try:
    ...
    # define new constraints and objective for minimization
    ...
    try:
        ...
        print("CSm_l", x2[mnet.reactions_id.index('R_CSm_l')])
        print("CSm_s", x2[mnet.reactions_id.index('R_CSm_s')])
        print("CSm_r", x2[mnet.reactions_id.index('R_CSm_r')])

        # new constraints for mCS reduction

        ub2[mnet.reactions_id.index('R_CSm_l')
            ] = x2[mnet.reactions_id.index('R_CSm_l')]*0.46
        ub2[mnet.reactions_id.index('R_CSm_s')
            ] = x2[mnet.reactions_id.index('R_CSm_s')]*0.46
        ub2[mnet.reactions_id.index('R_CSm_r')
            ] = x2[mnet.reactions_id.index('R_CSm_r')]*0.46

        rhs3 = [0] * (
                mnet.nb_metab + (len(mnet.reactions_id) * 2)) + [0] + [0]

        sense3 = "E" * mnet.nb_metab + "L" * (len(mnet.reactions_id) * 2) +\
                 "L" + "E"
        ...
        try:
            ...
            probFBA.set_problem_name("FBATomatoMinFluxSum_CSm")

            probFBA.objective.set_sense(probFBA.objective.sense.minimize)

            probFBA.linear_constraints.add(
                rhs=rhs3, senses=sense3,
                names=mnet.metabolites_id + mnet.reactions_id +
                mnet.reactions_id + ['other constraints']*2)

            probFBA.variables.add(
                obj=obj2, lb=lb2, ub=ub2,
                names=mnet.reactions_id +
                mnet.exchangereactions_id +
                mnet.reactions_id)
            ...
            probFBA.solve()
            flux_sum = probFBA.solution.get_objective_value()
            x3 = probFBA.solution.get_values()
            ...
            print("NH4", x3[mnet.reactions_id.index('R_EX_nh4_c_r')])
            print("NO3", x3[mnet.reactions_id.index('R_EX_no3_c_r')])

            print("co2_l", x3[mnet.reactions_id.index('R_EX_co2_e_l')])
            print("co2_s", x3[mnet.reactions_id.index('R_EX_co2_e_s')])
            print("co2_r", x3[mnet.reactions_id.index('R_EX_co2_e_r')])

            print("CSm_l", x3[mnet.reactions_id.index('R_CSm_l')])
            print("CSm_s", x3[mnet.reactions_id.index('R_CSm_s')])
            print("CSm_r", x3[mnet.reactions_id.index('R_CSm_r')])
            ...
            df1 = pd.DataFrame({
                ...
                'FBA value CSm': x3[0:len(mnet.reactions_id)],
                'FBA value WT': x2[0:len(mnet.reactions_id)]})

            df1 = df1.reindex(columns=['Reaction Name', 'Reaction Id',
                                       'Reaction Number',
                                       'Reaction Formula Name',
                                       'Reaction Formula Id',
                                       'FBA value CSm',
                                       'FBA value WT'])
            val_atp = str(cost)
            df1.to_excel('output/FBA_3comp_mCS.xlsx',
