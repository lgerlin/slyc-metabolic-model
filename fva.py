...
try:
    ...
    try:
        ...
        # save results
        print(len(x2))
        print(len(mnet.reactions_name + mnet.exchangereactions_name))
        df1 = pd.DataFrame({
            ...
            'FBA value': x2[0:len(mnet.reactions_name + mnet.exchangereactions_name)],
            'Absolute FBA value:': [abs(v) for v in x2[0:len(mnet.reactions_name + mnet.exchangereactions_name)]]})

        val_atp = str(mnet.cost)
        df1.to_excel('output/FBA_3comp_MinPhotons_MinFluxSum_' +
                     'TransportCost_' + val_atp +
                     '_RatioPS_' + str(mnet.leaf_stem_photosynthesis) +
                     '_Xyl_' + str(mnet.xyl) + '.xlsx',
...
rhsFVA = rhs2 + [probFBA2.solution.get_objective_value()*1]
senseFVA = sense2 + 'E'

constraint_minSumFlux_rows = [len(senseFVA)-1]*len(mnet.reactions_id)
constraint_minSumFlux_columns = list(range(len(mnet.reactions_id) + len(mnet.exchangereactions_id), len(lb2)))
...
for FVAreaction in FVAreactions:
    ...
    objFVA = [0]*(len(mnet.reactions_id) + len(mnet.exchangereactions_id) + len(mnet.reactions_id))
    objFVA[index_reaction] = 1

    try:

        ##################################################
        # FBA 3) et FBA 4) min et max flux of each fluxes
        #################################################

        probFVA = cplex.Cplex()

        probFVA.set_problem_name("FVATomatoMinFlux")
        ...
        probFVA.linear_constraints.set_coefficients(
            zip(mnet.stoichMat_rows + mnet.exchangestoichMat_rows + addstoichMat_rows + minstoichMat_rows + constraint_minSumFlux_rows,
                mnet.stoichMat_columns + mnet.exchangestoichMat_columns + addstoichMat_columns + minstoichMat_columns + constraint_minSumFlux_columns,
                mnet.stoichMat_values + mnet.exchangestoichMat_values + addstoichMat_values + minstoichMat_values + constraint_minSumFlux_values))
...
val_atp = str(mnet.cost)
dfFVA.to_excel('output/FVA_3comp_MinPhotons_MinFluxSum_' +
               'TransportCost_' + val_atp +
               '_RatioPS_' + str(mnet.leaf_stem_photosynthesis) +
               '_Xyl_' + str(mnet.xyl) + '.xlsx',
               sheet_name='FVAoutput', index=False)