...
#initializing variables to store results for ATP_cost variation
cost_values = [numb / 50.0 for numb in range(201)]
#cost_values = [numb / 50.0 for numb in range(11)]
pourc_gln_list = []
...
organic_metab_xylem = ['M_phe_L_c', 'M_tyr_L_c', 'M_asn_L_c', 'M_asp_L_c',
            'M_gln_L_c', 'M_arg_L_c', 'M_sucr_c', 'M_glc_D_c',
            'M_thr_L_c', 'M_pro_L_c', 'M_val_L_c','M_ile_L_c',
            'M_leu_L_c', 'M_lys_L_c', 'M_etoh_c','M_ala_L_c',
            'M_fum_c']
...
#
#                  Performing multiple FBA, changing ATP cost
#
...
#                      FLUX BALANCE ANALYSIS
    ...
    try:
        ...
        #save results
        photons.append(probFBA.solution.get_objective_value())
        statusphotons.append(probFBA.solution.get_status())
        ...
        try:
            ...
            # save results
            sumfluxes.append(probFBA2.solution.get_objective_value())
            statusminsumfluxes.append(probFBA2.solution.get_status())

            # determine gln proportion in xylem fluxes
            sum = 0
            gln = x2[mnet.reactions_name.index('Exchange_root_xylem_M_gln_L_c')]
            for metab in organic_metab_xylem:
                sum = sum + x2[mnet.reactions_name.index('Exchange_root_xylem_'+ metab)]

            try:
                pourcentage_gln = 100 * gln/sum
            except:
                pourcentage_gln = 'no organic molecules in xylem'
...
# save results
df1 = pd.DataFrame({'mol ATP per mol exchanged': cost_values,
                    'Pourc. GLN': pourc_gln_list,
                    'Import flux of photons': photons,
                    'Sum of fluxes': sumfluxes,
                    'Status min photons': statusphotons,
                    'Status min sum of fluxes': statusminsumfluxes})

df1.to_excel('output/FBA_3comp_ATP_transport_cost_effect.xlsx',
